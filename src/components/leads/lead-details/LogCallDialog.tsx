...
export const LogCallDialog = ({ open, onOpenChange, leadId }: LogCallDialogProps) => {
  ...
  const handleCreateReminder = () => {
    ...
    createReminderMutation.mutate({
      ...
      dueAt: followup.suggestedReminder.suggestedDueAt // Zod coerced this to a Date
    } as any, {
      onSuccess: () => {
        handleClose();
      }
  ...
  return (
    <Dialog open={open} onOpenChange={(val) => !val && handleClose()}>
      <DialogContent className={step === "review" ? "max-w-3xl max-h-[95vh]" : "sm:max-w-[425px]"}>
        <DialogHeader>
          <div className="flex items-center gap-3 mb-2">
             <div className="flex h-10 w-10 items-center justify-center rounded-full bg-primary/10 text-primary">
                {step === "log" ? <PhoneCall className="h-5 w-5" /> : <Sparkles className="h-5 w-5" />}
             </div>
          ...
          <DialogDescription>
            {step === "log" && "Record the outcome of your latest contact attempt with this lead."}
            {step === "suggest" && "Would you like AI to suggest a follow-up script and next steps?"}
            {step === "review" && "Review the AI-generated follow-up strategy below."}
          </DialogDescription>
        </DialogHeader>
        ...
        {step === "suggest" && (
          <div className="py-8 flex flex-col items-center justify-center text-center gap-4">
            ...
            <p className="text-sm text-muted-foreground max-w-[280px]">
              Call activity has been saved. The AI can help you draft a script and schedule the next follow-up.
            </p>
          </div>
        )}
          ...
          <div className="space-y-4 py-4">
            <Alert className="bg-primary/5 border-primary/20 text-primary-foreground border">
              ...
              <AlertDescription className="text-xs text-primary">
                AI suggestions can be wrong. Review and verify before taking action.
              </AlertDescription>
            </Alert>
              ...
              <div className="space-y-6">
                ...
                <Card>
                  <CardHeader className="flex flex-row items-center gap-2 pb-2">
                    ...
                    <CardTitle className="text-sm font-semibold">Suggested Call Script</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    <div>
                      <Label className="text-[10px] uppercase text-muted-foreground tracking-wider">Opening</Label>
                      <p className="text-sm mt-1 bg-muted/30 p-2 rounded border-l-2 border-primary italic">
                        "{followup.callScript.opening}"
                      </p>
                    </div>
                    <div>
                      <Label className="text-[10px] uppercase text-muted-foreground tracking-wider">Questions to Ask</Label>
                    ...
                    <div>
                      <Label className="text-[10px] uppercase text-muted-foreground tracking-wider">Objection Handlers</Label>
                      <div className="mt-1 grid gap-2">
                        {followup.callScript.objectionHandlers.map((obj, i) => (
                          <div key={i} className="text-xs p-2 rounded bg-muted/50 border">
                            <p className="font-semibold text-destructive">If: {obj.objection}</p>
                            <p className="mt-1 text-muted-foreground">Say: {obj.response}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <Card className="border-primary/20 bg-primary/5">
                    <CardHeader className="flex flex-row items-center gap-2 pb-2">
                      ...
                      <CardTitle className="text-sm font-semibold">Suggested Reminder</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-2">
                      <p className="text-sm font-bold">{followup.suggestedReminder.title}</p>
                      <div className="flex items-center gap-2 text-xs text-muted-foreground">
                        <Calendar className="h-3 w-3" />
                        {format(followup.suggestedReminder.suggestedDueAt, "PPP")}
                      </div>
                      <p className="text-xs italic text-muted-foreground">{followup.suggestedReminder.note}</p>
                    </CardContent>
                  </Card>
                </div>
              </div>
