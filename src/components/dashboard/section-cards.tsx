...
export async function SectionCards() {
  ...
  const baseWhere =
    profile.role === Role.AGENT ? { assignedToId: user.id } : {};

  const [totalLeads, inProgressLeads, wonLeads, remindersToday] =
    await Promise.all([
      prisma.lead.count({
        where: baseWhere,
      }),
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="@container/card shadow-sm hover:shadow-md transition-shadow border-border/50">
          <CardHeader className="pb-2">
            <CardDescription className="font-bold text-xs uppercase tracking-wider">
              Total Leads
            </CardDescription>
        ...
        <Card className="@container/card shadow-sm hover:shadow-md transition-shadow border-border/50">
          <CardHeader className="pb-2">
            <CardDescription className="font-bold text-xs uppercase tracking-wider text-blue-600 dark:text-blue-400">
              In Progress
            </CardDescription>
            <CardTitle className="text-3xl font-bold tabular-nums">
              {inProgressLeads}
        ...
        <Card className="@container/card shadow-sm hover:shadow-md transition-shadow border-emerald-500/10">
          <CardHeader className="pb-2">
            <CardDescription className="font-bold text-xs uppercase tracking-wider text-emerald-600 dark:text-emerald-400">
              Won Leads
            </CardDescription>
            <CardTitle className="text-3xl font-bold tabular-nums">
              {wonLeads}
      ...
      <div className="p-6 rounded-2xl border bg-orange-500/5 border-orange-500/10 flex flex-col md:flex-row items-center justify-between gap-4 transition-all hover:bg-orange-500/[0.07]">
        <div className="flex items-center gap-5">
          ...
          <div className="text-center md:text-left">
            <h3 className="text-lg font-bold text-foreground">
              You have {remindersToday}{" "}
              {remindersToday === 1 ? "task" : "tasks"} due today
            </h3>
        ...
        <Button
          asChild
          variant="outline"
          className="border-orange-500/20 hover:bg-orange-500/10 gap-2 font-bold px-6 py-5 rounded-xl shadow-sm transition-all group"
        >
          <Link href="/reminders">
            View All
